...
// qMRML includes
#include "qMRMLRtInversePlanningSegmentsModel_p.h"
...
//------------------------------------------------------------------------------
qMRMLRtInversePlanningSegmentsModelPrivate::qMRMLRtInversePlanningSegmentsModelPrivate(qMRMLRtInversePlanningSegmentsModel& object)
  : q_ptr(&object)
...
//------------------------------------------------------------------------------
qMRMLRtInversePlanningSegmentsModelPrivate::~qMRMLRtInversePlanningSegmentsModelPrivate()
{
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModelPrivate::init()
{
  Q_Q(qMRMLRtInversePlanningSegmentsModel);
  this->CallBack->SetClientData(q);
  this->CallBack->SetCallback(qMRMLRtInversePlanningSegmentsModel::onEvent);
  ...
  for (int i = 0; i < q->columnCount(); ++i)
  {
    ...
    {
      columnLabels << qMRMLRtInversePlanningSegmentsModel::tr("Opacity");
    }
    else if (i == q->nameColumn())
    {
      columnLabels << qMRMLRtInversePlanningSegmentsModel::tr("Name");
    }
    else if (i == q->layerColumn())
    {
      columnLabels << qMRMLRtInversePlanningSegmentsModel::tr("Layer");
    }
  }
  q->setHorizontalHeaderLabels(columnLabels);

  q->horizontalHeaderItem(q->nameColumn())->setToolTip(qMRMLRtInversePlanningSegmentsModel::tr("Segment name"));
  q->horizontalHeaderItem(q->visibilityColumn())->setToolTip(qMRMLRtInversePlanningSegmentsModel::tr("Segment visibility"));
  q->horizontalHeaderItem(q->colorColumn())->setToolTip(qMRMLRtInversePlanningSegmentsModel::tr("Segment color"));
  q->horizontalHeaderItem(q->opacityColumn())->setToolTip(qMRMLRtInversePlanningSegmentsModel::tr("Segment opacity (3D views)"));
...
//------------------------------------------------------------------------------
QStandardItem* qMRMLRtInversePlanningSegmentsModelPrivate::insertSegment(QString segmentID, int row/*=1*/)
{
  Q_Q(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
QString qMRMLRtInversePlanningSegmentsModelPrivate::getTerminologyUserDataForSegment(vtkSegment* segment)
{
  if (!segment)
  {
...
//------------------------------------------------------------------------------
// qMRMLRtInversePlanningSegmentsModel
//------------------------------------------------------------------------------
qMRMLRtInversePlanningSegmentsModel::qMRMLRtInversePlanningSegmentsModel(QObject *_parent)
  :QStandardItemModel(_parent)
  , d_ptr(new qMRMLRtInversePlanningSegmentsModelPrivate(*this))
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
  d->init();
}

//------------------------------------------------------------------------------
qMRMLRtInversePlanningSegmentsModel::qMRMLRtInversePlanningSegmentsModel(qMRMLRtInversePlanningSegmentsModelPrivate* pimpl, QObject* parent)
...
//------------------------------------------------------------------------------
qMRMLRtInversePlanningSegmentsModel::~qMRMLRtInversePlanningSegmentsModel() = default;

//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::setSegmentationNode(vtkMRMLSegmentationNode* segmentationNode)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
vtkMRMLSegmentationNode* qMRMLRtInversePlanningSegmentsModel::segmentationNode()const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
  return d->SegmentationNode;
}

// -----------------------------------------------------------------------------
QString qMRMLRtInversePlanningSegmentsModel::segmentIDFromIndex(const QModelIndex &index)const
{
  return this->segmentIDFromItem(this->itemFromIndex(index));
}

//------------------------------------------------------------------------------
QString qMRMLRtInversePlanningSegmentsModel::segmentIDFromItem(QStandardItem* item)const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
  ...
  QVariant segmentID = item->data(qMRMLRtInversePlanningSegmentsModel::SegmentIDRole);
  if (!segmentID.isValid())
  {
    return "";
  }
  return item->data(qMRMLRtInversePlanningSegmentsModel::SegmentIDRole).toString();
}
//------------------------------------------------------------------------------
QStandardItem* qMRMLRtInversePlanningSegmentsModel::itemFromSegmentID(QString segmentID, int column/*=0*/)const
{
...
//------------------------------------------------------------------------------
QModelIndex qMRMLRtInversePlanningSegmentsModel::indexFromSegmentID(QString segmentID, int column/*=0*/)const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
QModelIndexList qMRMLRtInversePlanningSegmentsModel::indexes(QString segmentID) const
{
  QModelIndex startIndex = this->index(0, 0);
  // QAbstractItemModel::match doesn't browse through columns, we need to do it manually
  QModelIndexList itemIndexes = this->match(
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::rebuildFromSegments()
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateFromSegments()
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
Qt::ItemFlags qMRMLRtInversePlanningSegmentsModel::segmentFlags(QString segmentID, int column)const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
  Q_UNUSED(segmentID);
  ...
  if (column != this->visibilityColumn() && column != this->layerColumn())
  {
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateItemFromSegment(QStandardItem* item, QString segmentID, int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
  if (!item)
  {
  ...
  //// Set ID
  item->setData(segmentID, qMRMLRtInversePlanningSegmentsModel::SegmentIDRole);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateItemDataFromSegment(QStandardItem* item, QString segmentID, int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
  if (!d->SegmentationNode)
  {
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateSegmentFromItem(QString segmentID, QStandardItem* item)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateSegmentFromItemData(QString segmentID, QStandardItem* item)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
  if (!d->SegmentationNode)
  {
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateItemsFromColumnIndex(int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
  if (column < 0)
  {
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateItemsFromSegmentID(QString segmentID)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//-----------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::onEvent(
  vtkObject* caller, unsigned long event, void* clientData, void* callData )
{
  ...
  qMRMLRtInversePlanningSegmentsModel* model = reinterpret_cast<qMRMLRtInversePlanningSegmentsModel*>(clientData);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::onSegmentAdded(QString segmentID)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
  if (!segmentID.isEmpty())
  {
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::onSegmentRemoved(QString removedSegmentID)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::onSegmentModified(QString segmentID)
{
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::onSegmentOrderModified()
{
  this->reorderItems();
  this->updateItemsFromColumnIndex(this->layerColumn());
}
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::onDisplayNodeModified()
{
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::reorderItems()
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::onItemChanged(QStandardItem* item)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
int qMRMLRtInversePlanningSegmentsModel::nameColumn()const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
  return d->NameColumn;
}
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::setNameColumn(int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
  d->NameColumn = column;
...
//------------------------------------------------------------------------------
int qMRMLRtInversePlanningSegmentsModel::visibilityColumn()const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::setVisibilityColumn(int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
int qMRMLRtInversePlanningSegmentsModel::colorColumn()const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::setColorColumn(int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
int qMRMLRtInversePlanningSegmentsModel::opacityColumn()const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::setOpacityColumn(int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
int qMRMLRtInversePlanningSegmentsModel::layerColumn()const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::setLayerColumn(int column)
{
  Q_D(qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
void qMRMLRtInversePlanningSegmentsModel::updateColumnCount()
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
...
//------------------------------------------------------------------------------
int qMRMLRtInversePlanningSegmentsModel::maxColumnId()const
{
  Q_D(const qMRMLRtInversePlanningSegmentsModel);
  int maxId = -1;
...
// --------------------------------------------------------------------------
QString qMRMLRtInversePlanningSegmentsModel::terminologyTooltipForSegment(vtkSegment* segment)
{
  if (!segment)
  {
